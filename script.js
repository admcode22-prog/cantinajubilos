...
const App = {
    ...
    // NOVAS PROPRIEDADES PARA FILTRO E BUSCA
    filtroVendas: 'todos',   // 'todos', 'entrega', 'retirada'
    buscaCliente: '',
    ...
    async carregarEventos() {
        try {
            ...
            for (const evento of eventos) {
                ...
                try {
                    const producaoRes = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.PRODUCAO}?evento_id=eq.${evento.id}&select=*`, { headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` } });
                    const producao = producaoRes.ok ? await producaoRes.json() : [];
                    
                    const comprovantesRes = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.COMPROVANTES}?evento_id=eq.${evento.id}&select=*`, { headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` } });
                    const comprovantes = comprovantesRes.ok ? await comprovantesRes.json() : [];
                    
                    const ingredientesRes = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.INGREDIENTES}?evento_id=eq.${evento.id}&select=*`, { headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` } });
                    const ingredientes = ingredientesRes.ok ? await ingredientesRes.json() : [];
                    
                    const vendasRes = await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.VENDAS}?evento_id=eq.${evento.id}&select=*`, { headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` } });
            ...
            for (const [data, event] of Object.entries(this.events)) {
                eventsBackup[data] = { ...event, comprovantes: event.comprovantes.map(c => ({ ...c, imagem: null })) };
            }
            ...
            try {
                ...
                if (localData) { this.events = JSON.parse(localData); console.log('Usando backup local'); }
                else this.events = {};
            } catch (e) { console.error('Erro ao carregar backup local:', e); this.events = {}; }
        }
    },
    ...
    async getOrCreateEventoId(data) {
        try {
            ...
            if (!checkResponse.ok) { console.error('Erro na consulta:', checkResponse.status); return null; }
            const existente = await checkResponse.json();
            if (existente && existente.length > 0) { console.log('✅ Evento existente ID:', existente[0].id); return existente[0].id; }
            console.log('➕ Criando novo evento para:', data);
            const novoEvento = { data: data, eventName: '', responsible: '', notes: '' };
            const createResponse = await fetch(`${SUPABASE_URL}/rest/v1/eventos`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}`, 'Prefer': 'return=representation' },
                body: JSON.stringify(novoEvento)
            });
            if (!createResponse.ok) { const errorText = await createResponse.text(); console.error('Erro ao criar:', createResponse.status, errorText); return null; }
            const criado = await createResponse.json();
            if (Array.isArray(criado) && criado.length > 0) return criado[0].id;
            else if (criado && criado.id) return criado.id;
            else { console.error('Resposta inesperada:', criado); return null; }
    ...
    gerarCalendario() {
        ...
        for (let dia = 1; dia <= totalDias; dia++) {
            ...
            html += `<div class="calendar-day ${temEvento ? 'has-event' : ''}" onclick="app.abrirDia('${dataKey}')">
    ...
    async abrirDia(dataKey) {
        ...
        this.mudarAba('vendas'); // Abre direto na aba de vendas
        this.esconderLoading();
    },
    ...
    carregarDadosEvento() {
        ...
        // Configurar listener de busca uma única vez
        const searchInput = document.getElementById('searchCliente');
        if (searchInput && !searchInput._listenerAdded) {
            searchInput.addEventListener('input', () => this.buscarVendas());
            searchInput._listenerAdded = true;
        }
        // Inicializar filtro "todos" ativo visualmente
        this.filtroVendas = 'todos';
        document.querySelectorAll('.btn-filter').forEach(btn => {
            if (btn.getAttribute('data-filtro') === 'todos') btn.classList.add('active');
    ...
    async salvarProducao() {
        ...
        try {
            ...
            if (this.producaoEditando) {
                ...
                if (itemExistente) vendidoAtual = itemExistente.vendido || 0;
            }
            ...
            if (this.producaoEditando) {
                await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.PRODUCAO}?id=eq.${this.producaoEditando}`, {
                    method: 'PATCH', headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` },
    ...
    atualizarListaProducao() {
        ...
        producao.forEach((item) => {
            ...
            html += `<div class="item-card" style="border-left-color: var(--success);">
                        <div class="item-info">
                            <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                <span class="item-name">${item.nome}</span>
                                <span class="item-badge" style="background: var(--primary);">R$ ${item.valor.toFixed(2)}</span>
                                ${disponivel > 0 ? `<span class="item-badge" style="background: var(--warning);">${disponivel} disponível</span>` : `<span class="item-badge" style="background: var(--danger);">Esgotado</span>`}
                            </div>
                        ...
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <button class="btn-icon" style="background: var(--primary);" onclick="app.mostrarFormProducao('${item.id}')" title="Editar">✏️</button>
                            <button class="btn-icon" style="background: var(--danger);" onclick="app.removerProducao('${item.id}')" title="Remover">🗑️</button>
                        </div>
                    </div>`;
        });
    ...
    atualizarSelectProdutos() {
        ...
        producao.forEach(item => {
            const disponivel = item.quantidade - (item.vendido || 0);
            if (disponivel > 0) options += `<option value="${item.id}">${item.nome} - R$ ${item.valor.toFixed(2)} (${disponivel} disp)</option>`;
        });
    ...
    atualizarSelectComprovantes() {
        ...
        comprovantes.forEach(comp => { const option = document.createElement('option'); option.value = comp.id; option.textContent = `${comp.nome} (R$ ${(comp.valorTotal || 0).toFixed(2)})`; select.appendChild(option); });
    ...
    async salvarIngrediente() {
        ...
        try {
            ...
            if (this.ingredienteEditando) {
                await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.INGREDIENTES}?id=eq.${this.ingredienteEditando}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` }, body: JSON.stringify(ingredienteData) });
            } else {
                await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.INGREDIENTES}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}`, 'Prefer': 'return=representation' }, body: JSON.stringify(ingredienteData) });
            }
            await this.carregarEventos();
    ...
    atualizarListaIngredientes() {
        ...
        ingredientes.forEach((item) => {
            ...
            html += `<div class="item-card ${compradoClass}" data-id="${item.id}" style="${item.comprado ? 'opacity: 0.8;' : ''}">
                        <div class="item-info">
                            <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                            ...
                            ${temComprovante ? `<div style="display: flex; align-items: center; gap: 4px; margin-top: 4px;"><span class="item-details" style="color: var(--primary);">📎 ${comprovante?.nome || 'Comprovante'}</span><button class="btn-icon" style="background: var(--primary); width: 24px; height: 24px; font-size: 0.8rem;" onclick="app.verComprovanteDoIngrediente('${item.comprovante_id}')" title="Ver comprovante">👁️</button></div>` : ''}
                        </div>
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <button class="btn-icon" style="background: var(--success);" onclick="app.toggleCompradoIngrediente('${item.id}')" title="Marcar como comprado">${compradoText}</button>
                            <button class="btn-icon" style="background: var(--primary);" onclick="app.mostrarFormIngrediente('${item.id}')" title="Editar">✏️</button>
                            <button class="btn-icon" style="background: var(--danger);" onclick="app.removerIngrediente('${item.id}')" title="Remover">🗑️</button>
                        </div>
                    </div>`;
        });
    ...
    atualizarListaIngredientesParaComprovante() {
        ...
        ingredientes.forEach(item => {
            ...
            const valorDisplay = item.valorTotal > 0 ? `R$ ${item.valorTotal.toFixed(2)}` : '💰 A definir';
            html += `<div style="display: flex; align-items: center; gap: 8px; padding: 8px; border-bottom: 1px solid var(--border); background: ${estaSelecionado ? 'rgba(249, 115, 22, 0.1)' : 'transparent'};">
                        <input type="checkbox" id="ingrediente_${itemId}" value="${itemId}" ${estaSelecionado ? 'checked' : ''} onchange="app.toggleIngredienteComprovante('${itemId}')" style="width: 20px; height: 20px; cursor: pointer;">
                        <div style="flex: 1;">
    ...
    atualizarListaComprovantes() {
        ...
        comprovantes.forEach((comp) => {
            ...
            html += `<div class="item-card" style="border-left-color: var(--success);">
                        <div class="item-info">
                            <div style="display: flex; align-items: center; gap: 6px;"><span class="item-name">📎 ${comp.nome}</span><span class="item-badge" style="background: var(--primary);">${totalItens} itens</span></div>
                            ...
                            ${totalItens > 0 ? `<div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed var(--border);"><div style="font-weight: 600; font-size: 0.8rem; margin-bottom: 4px;">Itens neste comprovante:</div>${itensList}</div>` : ''}
                        </div>
                        <div style="display: flex; align-items: center; gap: 6px;">
                            <button class="btn-icon" style="background: var(--primary);" onclick="app.verComprovante('${comp.id}')" title="Ver">👁️</button>
                            <button class="btn-icon" style="background: var(--warning);" onclick="app.mostrarFormComprovante('${comp.id}')" title="Editar">✏️</button>
                            <button class="btn-icon" style="background: var(--danger);" onclick="app.removerComprovante('${comp.id}')" title="Remover">🗑️</button>
                        </div>
                    </div>`;
        });
    ...
    limparFormVenda() {
        document.getElementById('vendaCliente').value = ''; document.getElementById('vendaProdutoId').value = ''; document.getElementById('vendaQtd').value = '1';
        document.getElementById('vendaValorUnit').value = ''; document.getElementById('vendaTotal').value = ''; document.getElementById('vendaFormaPagamento').value = 'dinheiro';
        document.getElementById('vendaValorPago').value = ''; document.getElementById('vendaEntrega').value = 'nao'; document.getElementById('vendaObs').value = '';
        document.getElementById('vendaDisponivel').value = ''; document.getElementById('vendaPendente').value = 'R$ 0,00';
    },
    ...
    async salvarVenda() {
        ...
        try {
            ...
            if (this.vendaEditando) {
                ...
                const vendaData = { evento_id: eventoId, cliente, produtoId: vendaAntiga.produtoId, produtoNome: vendaAntiga.produtoNome, quantidade: vendaAntiga.quantidade, valorUnit: vendaAntiga.valorUnit, valorPago, formaPagamento, entrega, observacoes, data: vendaAntiga.data };
                ...
                const produto = this.events[this.selectedDay].producao.find(p => String(p.id) === String(produtoId));
                const vendaData = { evento_id: eventoId, cliente, produtoId, produtoNome: produto.nome, quantidade, valorUnit, valorPago, formaPagamento, entrega, observacoes, data: new Date().toISOString() };
            ...
            this.carregarDadosEvento(); // atualiza lista com ordenação/filtros
    ...
    atualizarListaVendas() {
        ...
        // Aplicar filtro de entrega/retirada
        if (this.filtroVendas === 'entrega') vendas = vendas.filter(v => v.entrega === 'sim');
        else if (this.filtroVendas === 'retirada') vendas = vendas.filter(v => v.entrega === 'nao');
        ...
        // Ordenar alfabeticamente por cliente
        ...
        vendas.forEach((venda) => {
            ...
            const entregaText = venda.entrega === 'sim' ? '🚚 Entrega' : '🏬 Retirada';
            const entregaClass = venda.entrega === 'sim' ? 'entrega' : 'retirada';
            html += `<div class="venda-card" data-id="${venda.id}">
                        <div class="venda-header">
                            <span class="cliente-nome">${this.escapeHtml(venda.cliente)}</span>
                            <span class="entrega-badge ${entregaClass}">${entregaText}</span>
                        </div>
                        ...
                        <div class="venda-actions">
                            ...
                            ${venda.entrega === 'nao' ? `<button class="btn btn-success btn-sm" style="background: #10b981;" onclick="app.marcarEntregue('${venda.id}')">✅ Entregar</button>` : ''}
    ...
    async marcarEntregue(id) {
        ...
        try {
            ...
            const venda = this.events[this.selectedDay].vendas.find(v => String(v.id) === String(id));
            if (!venda) return;
            await fetch(`${SUPABASE_URL}/rest/v1/${TABLES.VENDAS}?id=eq.${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` }, body: JSON.stringify({ entrega: 'sim' }) });
            await this.carregarEventos();
            this.carregarDadosEvento();
        } catch (error) { console.error('Erro ao marcar entregue:', error); alert('Erro ao atualizar!'); }
        finally { this.esconderLoading(); }
    },
    ...
    mostrarCantinaHoje() { this.novaCantinaHoje(); document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active')); document.querySelector('.nav-item:nth-child(2)').classList.add('active'); },
    mostrarRelatorioGeral() {
        ...
        document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active')); document.querySelector('.nav-item:last-child').classList.add('active');
    }
};
